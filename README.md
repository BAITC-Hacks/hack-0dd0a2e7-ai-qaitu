...
Отдельный тест по коротким фрагментам предоставленных редакций №8 (2021) и №9 (2022) проверяет структуру из п. 3.4 и привязку обязанностей из пп. 5.4 и 5.5. Полные обезличенные файлы также проверены локально: PDF редакции №8 дал 2 подразделения и 57 выделенных функций, DOCX редакции №9 — 4 подразделения и 54 функции. Результат: 2 сохранённых и 2 созданных подразделения, 3 потенциальных пересечения, без подтверждённой потери среди выделенных функций. Исходные документы и JSON-отчёт не включены в репозиторий.
...
- Локальный анализ лексический и может пропускать сильные перефразировки; для этого предусмотрен LLM-проход. Проверка существования `source_id` не доказывает корректность смыслового вывода — его проверяет сотрудник.

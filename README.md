...
3. Начните с обзора и **«Заключения»**: на экране кратко показаны охват, индикаторы рисков и изменения структуры. В разделе **«ИИ-сравнение»** проверьте смысловые изменения, цитаты обеих редакций и статус независимой проверки.
...
В боковой панели выберите **«Эффективность и оптимизация»**. Это отдельный слой: сравнение положений не запускает обработку отчётов или дополнительные запросы к ИИ.

1. Для демонстрации выберите тестовые данные и запустите демо. Генератор создаёт 9 Excel-файлов с месячными рядами 2021–2023: 2 052 значения по 19 метрикам. Каждый экран, график и строка выгрузки помечены **«ТЕСТОВЫЕ ДАННЫЕ»**. Заложенные эффекты не являются результатами работы реальной компании.
2. Для реального анализа загрузите XLSX/XLSM, DOCX или PDF с текстовым слоем либо положите отчёты в `data/reports/`. Проверьте извлечённые числа и очередь неоднозначных сопоставлений. Неизвестные метрики требуют подтверждения как `custom_*`. Число, период, подразделение и источник сохраняются вместе.
3. После сравнения реальных положений перенесите структурные события, подтвердите дату вступления в силу, названия подразделений в отчётах и механизм связи с метрикой. Для контрольной группы требуется отдельное подтверждение её незатронутости. Подтверждённые механизмы кэшируются по исходным цитатам.
4. Запустите расчёт. Доступны графики, сравнения, объяснения и альтернативы, прогнозы с проверкой фактом, рекомендации с решениями «принять / отклонить / обсудить» и накопленная база знаний. Скачайте JSON или Markdown с источниками.

По умолчанию модуль рассчитывает показатели локально. Опция ИИ использует серверные настройки OpenAI для извлечения чисел из текста, анализа связей и независимой проверки оппонентом. Только явное действие пользователя отправляет данные в API. Ошибка или неполный ответ ИИ не отменяют локальные вычисления. Уровень уверенности задаёт код; ИИ не может его повысить сверх результата правил.

```bash
# Отключить весь дополнительный раздел и его импорты
METRICS_MODULE=off python -m streamlit run app.py

# Сгенерировать отдельный синтетический комплект
python scripts/gen_synthetic_reports.py --output data/synthetic_reports
```

Промежуточные результаты сохраняются в `runs/<run_id>/metrics/M1_…M9_*.json`, база знаний и решения — в `data/kb.sqlite`. Файлы отчётов, результаты, база и локальный ключ исключены из Git и Docker-контекста. Реальная и синтетическая базы разделены. Повторное рассмотрение одного события обновляет существующий случай, а не увеличивает число наблюдений.

Ограничения и соответствие требованиям: [описание модуля](docs/METRICS_MODULE_IMPLEMENTATION.md). В текущем ядре нет таймлайна, симулятора и редактора правок: приложение не выдумывает их результаты. Есть явно обозначенные прогнозы синтетического сценария и ручные гипотезы; ручные гипотезы не входят в точность прогнозов агента.

...
Рабочий экран строится вокруг трёх действий: понять изменения, выбрать вопрос
для проверки и открыть подтверждающие цитаты. Сводка компактная, вопросы
показываются по пять, легенда и подробные пояснения раскрываются по запросу.
Матрица и исходные цитаты остаются на спокойном плотном фоне. Светлая, тёмная и системная темы доступны через меню Streamlit
...
После анализа верхняя панель ведёт к обзору, вопросам для проверки, матрице, структуре,
источникам и экспорту без повторного запуска анализа. Матрица показывает по 10
строк на странице и использует широкую текстовую колонку и полные названия
подразделений. Кнопка **«Развернуть»** открывает матрицу в большом окне поверх
отчёта, с текущими фильтрами и отдельными страницами по 25 строк. Escape, крестик
или нажатие вне окна возвращают к отчёту, сохраняя его страницу и выбор.
Основной размер для проверки композиции — 1920×1080, при этом ширина остаётся
адаптивной. Блоки назначений
...
Локальный `theme-bridge.html` передаёт палитру в нативные всплывающие окна Streamlit;
текст документов в JavaScript не передаётся.
...
### Проверка проекта и согласование (ветка features/document-review)

В боковой панели добавлены режимы **«Проверка проекта»**, **«Разбор реорганизации»**
и **«Пакет для СД / Комитета»**. Их переключение не повторяет анализ и API-вызовы.
Для одной редакции выберите «Проверка проекта», загрузите её только в «После изменений»
и нажмите «Проверить проект»: фиктивного сравнения с пустым прошлым не будет.

Локальный линтер проверяет пустые пункты, разрывы/повторы нумерации, отсутствующие
цели ссылок и смысловые кандидаты «право ↔ запрет». Это не полный юридический
анализ; проверки противоречий через LLM и сокращений ещё нет.

В чек-листе решения требуют имени и комментария. Для отметки «исправлено» укажите
редакцию и пункт; закрытие требует следующего отдельного подтверждения.
Журнал сохраняется в `.qaitu/reviews.sqlite3`, путь можно задать через
`QAITU_REVIEW_DB`. В контейнере подключите постоянный том для каталога базы.
Имя эксперта самозаявленное, а зелёный статус означает только закрытый чек-лист,
не разрешение на утверждение. Для публичного многопользовательского доступа
потребуется аутентификация и разделение данных.

Скачиваются **лист согласования Word/PDF** и **JSON-журнал решений**. Это отдельные
документы, не изменения исходного положения и не Word tracked changes.
Полный объём, отложенные пункты FEATURES и расхождения с эталоном описаны в
[docs/FEATURES_STATUS.md](docs/FEATURES_STATUS.md).

### Сравнение двух комплектов
...
3. Начните с обзора и раздела **«Заключение»**: на экране кратко показаны охват, индикаторы рисков и изменения структуры. Для руководителя скачайте краткий PDF; выводить весь каталог цитат на защите не требуется.
...
5. Скачайте отчёт для проверки командой: краткий или полный PDF, аналитический Excel либо JSON. Markdown оставлен в «Дополнительно» как технический формат.
...
Четыре основных кнопки находятся в разделе «Экспорт отчёта»:

| Формат | Что внутри |
|---|---|
| Краткий PDF | Первая страница — фактическая сводка и вывод; затем структура, до 10 кандидатов с уверенностью от 40% по убыванию оценки, до 8 передач и до 8 новых закреплений |
| Полный PDF | Сводка, структура, все находки (включая слабые), все передачи и новые закрепления; приложение А — полная матрица в альбомной ориентации; приложение Б — все исходные фрагменты |
| Excel | Восемь листов: Сводка, Изменения структуры, Риски, Матрица функций, Потери, Переданные функции, Новые функции, Источники |
| JSON | Полный машиночитаемый результат без изменения схемы экспорта |

Краткий PDF не является конвертацией Markdown: у него отдельная вёрстка и ограниченный
объём содержания. На оригинальной паре редакций №8/№9 он занимает 6 страниц; это не жёсткая
гарантия для любых документов. Сокращённые цитаты помечаются явно, рядом есть исходный
файл, пункт/страница и ID. Счётчики отражают весь результат, а над выборкой указано,
сколько записей показано. Числа из примеров не зашиты в шаблон.

Полный PDF может быть значительно длиннее: он сохраняет **все** назначения и весь
каталог фрагментов, включая контекст. В нём есть закладки разделов, повторяемые заголовки
таблиц и внутренние ссылки на источники. Для быстрого чтения используйте краткий PDF,
для фильтрации большого объёма — Excel. Полный PDF предназначен для подробной сверки.

В Excel матрица нормализована: одна строка — одно назначение в одной редакции одному
владельцу. Общий ID строки связывает назначения «до/после». Поэтому 182 строки матрицы
могут разворачиваться в 427 записей Excel: это не дополнительные функции. Доступны
автофильтры, закрепление заголовков и переходы из назначений в лист источников.
Уверенность хранится числом с процентным форматом: можно фильтровать `>70%`.
Пустая оценка означает отсутствие расчёта, а не 0%. Тексты не превращаются в формулы;
слишком длинные ячейки сохраняются последовательными частями с колонкой «Часть записи».

PDF/Excel формируются локально **только при нажатии кнопки**, без API, внешних шрифтов,
сохранения загруженных документов в репозиторий или общего межпользовательского кэша.
Фильтры интерфейса не сокращают экспорт. Кириллические шрифты DejaVu включены в проект
с лицензией (`qaitu/static/fonts/LICENSE.txt`); экспорт одинаково доступен в Docker,
Linux и Windows. После обновления существующей установки выполните:
...
HTML-экспорт пока не добавлен: основные читаемые форматы — PDF и Excel.
...
Синий цвет обозначает изменения назначений, красный — отсутствие найденного преемника, жёлтый — возможное пересечение. `?` означает неопределённость, `—` — отсутствие подразделения в перечне выбранной версии, точка — отсутствие прямого назначения. Подсказка ячейки уточняет значение отметки. Матрица выводится страницами; фильтры не меняют глобальный вывод и полный экспорт.

...
def assess_result(
    ...
    def support(left: Function, right: Function) -> float:
        ...
        norm_weight = 1.0 if left.norm_type == right.norm_type else .65
        return (.55 * text + .20 * action + .25 * obj) * norm_weight
    ...
    def loss_score(row: MatrixRow) -> ConfidenceAssessment:
        ...
        strongest_pair = max(pairs, key=lambda pair: support(*pair), default=None)
    ...
    for finding in result.findings:
        ...
        if finding.kind == "loss" and row and row.assessment:
            ...
            ids = {source.id for source in finding.sources}
            items = [f for f in after if f.source.id in ids and f.norm_type == "duty"]
            pairs = [(a, b) for a in items for b in items if a.role == "execute" and b.role == "control"
